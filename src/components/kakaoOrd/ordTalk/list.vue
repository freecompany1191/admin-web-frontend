			...
			<div class="inner-search-box floatLeft">
				...
				<div>
					<ul>
						...
						<li>
							<label><input type="radio" name="send_time_type" id="send_time_type" v-model="send_time_type" value="" checked/>전체</label>
							<label><input type="radio" name="send_time_type" id="send_time_type" v-model="send_time_type" value="3"/>주문완료</label>
							<label><input type="radio" name="send_time_type" id="send_time_type" v-model="send_time_type" value="4"/>요일지정</label>
						</li>
					</ul>
				</div>
			</div>
...
export default {
	...
	data () {
		return {
			pageTitle: '주문톡관리'
			, pageTitle_sub: '주문톡 목록을 설정하실수 있습니다'
	...
	methods: {
		...
		getTalkList: function() { 
			...
			var IN_VALUE = tmp0 + "│"		// 페이지_요청번호
					...
					+ tmp7 + "│"			// 검색_발송_시점_구분 (NULL: 전체, 3: 완료, 4: 요일지정)
					;
		...
		schView: function(idx) {
			this.$router.push({ name: 'ordTalkView', params: {no: idx} })
		}
	},
	watch : {
		//데이터가 변할시마다 함수 호출
		'$route' (to, from) {
			if (to.name == "ordTalkList") {
